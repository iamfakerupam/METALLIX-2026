...
// ── Classes moved OUTSIDE useEffect (fixes "Inline class declarations not supported") ──
...
class GLProgram {
  ...
  constructor(
    ...
    this.program = GLProgram.createProgram(gl, vertexShader, fragmentShader);
    this.uniforms = this.program ? GLProgram.getUniforms(gl, this.program) : {};
  }
...
class GLMaterial {
  ...
  setKeywords(
    ...
    let hash = 0;
    for (const kw of keywords) hash += hashCodeUtil(kw);
    ...
    if (program == null) {
      ...
      program = GLProgram.createProgram(gl, this.vertexShader, fragmentShader);
      this.programs[hash] = program;
    }
    if (program === this.activeProgram) return;
    if (program) {
      this.uniforms = GLProgram.getUniforms(gl, program);
    }
...
// Utility: hash outside class so GLMaterial can use it
function hashCodeUtil(s: string): number {
...
// ── Red / violet / black palette ──
// Returns one of a fixed set of hues: deep red, crimson, violet, purple, near-black
const THEME_COLORS: ColorRGB[] = [
  { r: 0.18, g: 0.00, b: 0.00 }, // near-black red
  { r: 0.35, g: 0.00, b: 0.02 }, // deep crimson
  { r: 0.55, g: 0.00, b: 0.04 }, // crimson
  { r: 0.40, g: 0.00, b: 0.25 }, // red-violet
  { r: 0.22, g: 0.00, b: 0.30 }, // dark violet
  { r: 0.14, g: 0.00, b: 0.22 }, // deep purple
  { r: 0.30, g: 0.00, b: 0.18 }, // magenta-black
  { r: 0.50, g: 0.00, b: 0.00 }, // pure red
];

function generateThemeColor(): ColorRGB {
  const base = THEME_COLORS[Math.floor(Math.random() * THEME_COLORS.length)];
  // Small random variation so it doesn't look flat
  const jitter = 0.08;
  return {
    r: Math.max(0, base.r + (Math.random() - 0.5) * jitter),
    g: Math.max(0, base.g + (Math.random() - 0.5) * 0.01),
    b: Math.max(0, base.b + (Math.random() - 0.5) * jitter),
  };
}
...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement): {
      ...
      if (isWebGL2) {
        ...
        halfFloat = gl.getExtension('OES_texture_half_float') as OESTextureHalfFloat | null;
        supportLinearFiltering = !!gl.getExtension('OES_texture_half_float_linear');
      }

      gl.clearColor(0, 0, 0, 1);

      // halfFloatTexType: properly typed, no `any`
      const halfFloatTexType: number = isWebGL2
        ? (gl as WebGL2RenderingContext).HALF_FLOAT
        : halfFloat?.HALF_FLOAT_OES ?? 0;

      // formatRGBA / formatRG / formatR: typed as TextureFormat | null, no `any`
    ...
    function compileShader(
      ...
      const src = addKeywords(source, keywords ?? null);
      const shader = _gl.createShader(type);
      if (!shader) return null;
      _gl.shaderSource(shader, src);
      _gl.compileShader(shader);
      if (!_gl.getShaderParameter(shader, _gl.COMPILE_STATUS)) {
        console.trace(_gl.getShaderInfoLog(shader));
      }
      return shader;
    }
    ...
    // ── Programs (using renamed class GLProgram) ──
    ...
    const gradSubtractProgram = new GLProgram(gl, baseVertexShader, gradientSubtractShader);
    ...
    function updateKeywords() {
      ...
      displayMaterial.setKeywords(gl, displayKeywords, compileShader);
    }
    ...
    function updateColors(dt: number) {
      ...
      if (colorUpdateTimer >= 1) {
        ...
        pointers.forEach(p => { p.color = generateThemeColor(); });
      }
    }
    ...
    function clickSplat(pointer: Pointer) {
      ...
      // Boost brightness for click splats
  ...
  function updatePointerDownData(pointer: Pointer, id: number, posX: number, posY: number) {
      ...
      pointer.color = generateThemeColor();
    }
...
function updatePointerMoveData(pointer: Pointer, posX: number, posY: number, color: ColorRGB) {
    ...
    function handleFirstMouseMove(e: MouseEvent) {
      const pointer = pointers[0];
      updatePointerMoveData(pointer, scaleByPixelRatio(e.clientX), scaleByPixelRatio(e.clientY), generateThemeColor());
      updateFrame();
